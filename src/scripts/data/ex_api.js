...
function convertToRupiah(priceString) {
  ...
  if (priceString.toLowerCase().includes('rp') || priceString.toLowerCase().includes('idr')) {
    return priceString;
  }
...
const Phones = {
  ...
  async getPhoneByBrand(brand, maxPrice = 0) {
    try {
      ...
      const phonesByBrand = data.filter(phone => 
        phone.phone_brand.toLowerCase() === brand.toLowerCase()
      );
      ...
      const bestPhones = phonesByBrand
        .filter(phone => phone.specs && phone.specs.Memory && phone.specs["Main Camera"])
        .map(phone => {
          ...
          const cameraText = phone.specs["Main Camera"]?.Dual || phone.specs["Main Camera"]?.Triple || "0 MP";
        ...
        .map(phone => ({
          ...
          kamera: phone.specs["Main Camera"]?.Dual || phone.specs["Main Camera"]?.Triple || "N/A",
          baterai: phone.specs.Battery?.["Non-removable Li-Po"] || phone.specs.Battery?.Type || "N/A",
          chipset: phone.specs.Platform?.Chipset || phone.specs.Platform?.CPU || "N/A",
          display: phone.specs.Display?.Size || phone.specs.Display?.Type || "N/A"
        }));
  ...
  async getCommentByBrand(brand) {
    try {
      ...
      const relevantReviews = allReviews
        .filter(item => {
          const brandNameInText = item.text.toLowerCase().includes(brand.toLowerCase());
  ...
  async getCommentByPhoneModel(brand, phoneModel) {
    try {
      ...
      const modelKeywords = phoneModel.toLowerCase()
        .replace(/[^\w\s]/g, '') // hapus karakter khusus
        .replace(/\s+/g, ' ')     // normalize spasi
        .split(' ')
        .filter(word => word.length > 1) // ambil kata minimal 2 huruf
        .filter(word => !['hp', 'phone', 'smartphone', 'android', 'ios'].includes(word)); // hapus kata generik
      ...
      console.log('Found', modelSpecificReviews.length, 'specific reviews for', phoneModel);
      ...
      const brandOnlyReviews = allReviews
        .filter(item => {
          const textLower = item.text.toLowerCase();
          return textLower.includes(brand.toLowerCase()) && item.sentiment === 'positive';
        })
        .sort((a, b) => b.like_count - a.like_count)
        .slice(0, 5)
        .map(item => ({
      ...
      console.log('Fallback to brand reviews:', brandOnlyReviews.length, 'found');
